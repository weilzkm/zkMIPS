...
impl<F: PrimeField32> MachineAir<F> for MiscInstrsChip {
    ...
    fn generate_trace(
        ...
        let chunk_size = std::cmp::max((input.instrs_record.misc_events.len()) / num_cpus::get(), 1);
        let nb_rows = input.instrs_record.misc_events.len();
        ...
        let blu_events = values
            ...
            .map(|(i, rows)| {
                ...
                rows.chunks_mut(NUM_MISC_INSTR_COLS).enumerate().for_each(|(j, row)| {
                    ...
                    if idx < input.instrs_record.misc_events.len() {
                        let event = &input.instrs_record.misc_events[idx];
    ...
    fn included(&self, shard: &Self::Record) -> bool {
        if let Some(shape) = shard.shape.as_ref() {
            ...
            !shard.instrs_record.misc_events.is_empty()
        }
    }
}
